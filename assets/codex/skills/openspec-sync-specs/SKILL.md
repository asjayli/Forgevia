...
**Review protocol:** A sync request authorizes updates to the named change's corresponding main specs, but not archive, commit, push, or release. Use `spawn_agent` for an independent reviewer different from the plan or sync-result producer. Every review package contains:
