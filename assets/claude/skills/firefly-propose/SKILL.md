---
name: firefly-propose
description: Use when the user explicitly asks firefly to turn a requirement description or a specified file into a new OpenSpec change proposal.
---

# firefly Propose

Use this skill when the user explicitly wants firefly to produce a new OpenSpec change from a requirement description or a specified file.
...
- Report that the standalone `propose` command reached its apply-ready terminal condition, then provide `firefly implement <resolved-change>` as the exact next command.
...
- When this proposal is a phase of an already-authorized complete firefly delivery, return control internally with the resolved change name; do not emit a proposal completion response or wait for confirmation.
...
- After creating the change artifacts, run `"${CLAUDE_HOME:-$HOME/.claude}/firefly/bin/firefly" validate --root <project-root>`. Do not report the proposal as strictly valid if this command fails.
