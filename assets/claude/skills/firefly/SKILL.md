---
name: firefly
description: Use when the user explicitly asks to use firefly to run the full OpenSpec plus superpowers plus review plus Playwright workflow instead of invoking the underlying skills manually.
---

# firefly

firefly is an explicit orchestration skill. It does not replace OpenSpec, superpowers, requesting-code-review, or playwright-interactive. It coordinates them.

## When To Use

Use firefly only when the user explicitly asks for it, such as:

- `use firefly`
- `run this through firefly`
- `use the firefly workflow`

Do not auto-trigger firefly just because a coding request exists.

## Preconditions

Before using firefly, verify:

- `openspec` is installed and available
- required `superpowers` skills are installed
- firefly-managed Claude assets are present under `~/.claude`
...
firefly uses deterministic routing:

- A named subcommand authorizes that command's documented terminal condition.
- A firefly request with a requirement but no named subcommand authorizes the complete delivery workflow by default. Clarify only consequential ambiguity, then propose, implement, run browser verification when relevant, and perform the final independent review.
...
**Claude Code continuous controller:** Until the workflow reaches every final completion gate, receives a valid `ESCALATE`, or the user explicitly stops it, the controller MUST continue execution. It MUST NOT send a `final` response after a child-agent callback, a review `REVISE`, or the end of a phase's verification.

**Claude Code child-agent lifecycle:** After dispatching an in-scope child with the native `Task` mechanism, retain control and collect its lifecycle through Claude Code's native agent handling. When a callback arrives, automatically continue with its repair, verification, or next workflow phase, then continue tracking every remaining in-scope child. Do not emit a `final` response, completion summary, or user-facing wait request while an in-scope child remains active, except for an explicit user interruption or a valid `ESCALATE` boundary.
...
Purpose:
- check the global firefly environment
...
- for the Claude firefly path, prefer `--tools codex,claude`

Behavior:
- route to firefly's environment checks and bootstrap flow
- do not take ownership of project source files

### `firefly doctor`

Purpose:
- inspect the current Claude environment for missing or drifted firefly-managed assets

Behavior:
- use the firefly doctor flow
- report `OK`, `MISS`, or `DRIFT`

### `firefly repair`

Purpose:
- repair missing or drifted firefly-managed assets

Behavior:
- use the firefly repair flow
- preserve backups before replacement

### `firefly implement <change>`
...
- for standalone `firefly propose`, report apply-ready as the command's normal terminal condition and provide `firefly implement <resolved-change>` as the exact next command without asking whether to proceed
...
- pass the Mermaid output to `firefly-draw.sh`
...
### 2. Use the firefly-modified superpowers path

When implementation planning or execution is needed, prefer the firefly-managed variants of:
...
firefly may help detect whether a repository has been initialized with OpenSpec and may invoke `openspec init` when the user wants that help.

firefly does not take ownership of project source files or silently overlay project-local workflow files.

## Boundaries

- firefly orchestrates; it does not manually duplicate every underlying skill body.
- firefly should keep the user on one coherent workflow, not invent side workflows.
- If a lower-level skill is clearly the right direct tool for the current step, firefly should say so and use it.
- firefly manages global workflow environment and invocation patterns, not user project code.
