...
- Run `"${CLAUDE_HOME:-$HOME/.claude}/firefly/bin/firefly" validate --root <project-root>` before the review. Treat a non-zero result as a blocking OpenSpec finding and report its file-level output.
