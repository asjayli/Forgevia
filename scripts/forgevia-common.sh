...
# ~/.{claude,codex}/forgevia/bin/ load the same helpers from an adjacent path.
#
...
# Fails loudly when neither provider is available; Forgevia never silently
# downgrades to a weaker hash.
forgevia_sha256_file() {
  local path="$1"
  local line
...
# forgevia_sha256_file so a missing provider stays attributable whether it
