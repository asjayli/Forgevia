...
|----------|---------|-------------------|-----------------|-----|
| OpenSpec (`@fission-ai/openspec`) | spec-driven change workflow CLI | override targets `1.4.1` | `1.5.0` (`1.6.0-beta.1`) | https://www.npmjs.com/package/@fission-ai/openspec |
...
> Note: Forgevia's OpenSpec override is a content snapshot taken against `1.4.1`. When the upstream OpenSpec version differs from the snapshot, the installer and doctor skip the overlay protectively to avoid downgrading upstream. To restore Forgevia's customizations on a newer upstream, update the override snapshot together with `overrideTargetVersion` in `manifests/*.json`.
