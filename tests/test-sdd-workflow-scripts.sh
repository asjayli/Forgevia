...
for path in "${SDD_SKILLS[@]}"; do
  ...
  assert_contains "$contents" '`scripts/review-package BASE WORKTREE`'
  assert_contains "$contents" 'includes committed, staged, unstaged, and untracked task changes'
  assert_in_order "$contents" \
...
worktree_review_output="$(cd "$repo_dir/nested" && "$REVIEW_PACKAGE" "$head_sha" WORKTREE)"
worktree_review_path="${worktree_review_output#wrote }"
worktree_review_path="${worktree_review_path%%: *}"
assert_contains "$worktree_review_output" "wrote $repo_dir/.superpowers/sdd/review-"
assert_contains "$worktree_review_output" "worktree"
assert_file_exists "$worktree_review_path"
worktree_review_contents="$(<"$worktree_review_path")"
assert_contains "$worktree_review_contents" "# Review package: ${head_sha}..WORKTREE"
assert_contains "$worktree_review_contents" "uncommitted change"
assert_contains "$worktree_review_contents" "new-file.txt"
assert_contains "$worktree_review_contents" "new untracked file"
