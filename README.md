...
- `firefly`：firefly 工作流的总入口。
- `firefly-init`：为当前项目准备 firefly 工作流环境。
- `firefly-think`：在开发前梳理和澄清需求。
- `firefly-propose`：将需求描述或指定文件转换为新的变更提案。
- `firefly-implement`：针对指定的活跃变更执行结构化开发。
- `firefly-tasks`：查看所有活跃变更下未完成的任务。
- `firefly-review`：对当前工作发起一次聚焦评审。
- `firefly-verify-web`：在浏览器中验证 Web 侧行为。
- `firefly-draw`：为功能、链路或接口生成交互时序图。
- `openspec-sync-specs`：将指定 change 的 delta specs 同步至主规格，保持 change 活跃且不归档。
- `firefly-archive`：归档一个已完成的活跃变更。
- `firefly-doctor`：检查 firefly 环境是否健康。
- `firefly-repair`：修复缺失或漂移的 firefly 受管资源。

## 如何使用

### 完整版链路

`firefly` 是总入口。可以用显式子命令控制单个阶段，也可以直接提供要交付的需求，让 firefly 自动编排方案、开发、评审和验证。
...
1. `firefly init`
   适合在新仓库开始使用时执行。它会检查当前项目是否已具备 firefly 工作流所需的基础文件；如果缺失，就补齐初始化内容。若这个仓库需要同时支持 Codex 和 Claude，建议按 `codex,claude` 初始化项目模板。
2. `firefly doctor`
   用于检查全局环境状态。它会告诉你当前安装的 firefly 资源是否正常、缺失，或者已经发生漂移。
3. `firefly repair`
   当 `doctor` 发现问题时使用。它会把缺失或漂移的 firefly 受管文件恢复到正确状态。
4. `firefly draw`
...
firefly 把需求梳理、结构化开发、代码评审、效果验证和最终归档串成一条一致的交付流程。

完整交付结束后 change 保持 active；`propose` 达到 apply-ready 只是内部阶段转换，不会停下来再次询问。显式执行 `firefly propose` 时则以 apply-ready 为命令终态，并返回可直接执行的 `firefly implement <change>`，不会用“中断”或“未授权”描述正常完成，也不会追加是否继续的确认问题。

### 中文 OpenSpec 严格校验

firefly 通过 `firefly validate` 提供中文 OpenSpec 严格校验，用于在不修改源规格的前提下支持需求正文中的“必须、不得、禁止、应当”。它先校验中文强制词，再在临时副本中注入 `MUST` 并运行原生严格校验：

```bash
firefly validate --root <项目根目录>
```

安装器会将平台无关的命令安装到 `~/.local/bin/firefly`；确保该目录在 PATH 中后，可直接运行 `firefly validate --root <项目根目录>`。当前命令分发器还支持 `init`、`tasks`、`draw`、`doctor` 和 `repair`。当 Codex 与 Claude 都已安装时，`doctor` 与 `repair` 会检查两个运行时；其他命令优先使用 Codex 运行时。适配器同时校验主规格和活跃变更，且只支持 `spec-driven` schema；不支持中文章节或标题，必须继续使用 `## Requirements`、`### Requirement:` 和 `#### Scenario:` 等 OpenSpec 结构关键字。

## 第三方资产与许可

- `playwright-interactive`：来源于上游，采用 Apache License 2.0（© Microsoft Corporation）。firefly 重新分发时保留了其 `LICENSE.txt` 与 `NOTICE.txt`。
- `mermaid-diagram-specialist`：firefly 原创技能。
- `superpowers` 与 `OpenSpec`：作为上游依赖分别安装，firefly 仅在其上叠加受管覆盖文件，详见 `INSTALL.claude.md` / `INSTALL.codex.md`。

## 上游依赖

firefly 在下列上游之上叠加受管定制。覆盖快照针对的版本与上游最新版本的差异，决定是否需要适配。

| 上游 | 用途 | firefly 基准 | 上游最新 | 地址 |
|------|------|---------------|----------|------|
...
| mermaid-cli (`mmdc`) | `firefly-draw` 渲染 SVG 的运行时依赖 | 运行时工具 | — | https://github.com/mermaid-js/mermaid-cli |
| ripgrep (`rg`) | `firefly-tasks` 扫描任务（可选，缺失回退 grep） | 运行时工具 | — | https://github.com/BurntSushi/ripgrep |

> 说明：firefly 对 OpenSpec 的覆盖是针对 `1.6.0` 的内容快照，安装器固定安装此版本。当本地 OpenSpec 版本与快照不一致时，安装器与 doctor 会保护性跳过覆盖，避免降级上游；升级时必须同步更新覆盖快照、安装版本和 `manifests/*.json` 中的 `overrideTargetVersion`。
